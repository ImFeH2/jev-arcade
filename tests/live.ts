...
import { createGame, placements, sequence } from '@/games/tetris/rules'
...
const response = await fetch('http://127.0.0.1:3001/api/decision', {
  ...
  body: JSON.stringify({ game: 'tetris', decisionId: 'live:0', board: game.board, kind: game.piece.kind, next: pieces.slice(1, 4) }),
})
...
assert.ok(placements(game).some(option => option.id === result.action))
assert.ok(result.inputTokens > 0)
console.info(`Official Jev decision verified: ${result.action}; input tokens: ${result.inputTokens}`)

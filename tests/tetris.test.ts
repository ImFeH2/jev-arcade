...
import { createGame, drop, fits, interval, lock, move, placements, rotate, sequence, shape, tick, WIDTH, HEIGHT } from '@/games/tetris/rules'
...
test('seven-bag sequences are deterministic, complete, and independent', () => {
  ...
  for (let i = 0; i < 140; i += 7) assert.deepEqual([...pieces.slice(i, i + 7)].sort(), [0, 1, 2, 3, 4, 5, 6])
...
test('every proposed placement is valid through gravity on changing boards', () => {
  ...
  for (let i = 0; i < 100 && !game.over; i++) {
    ...
    for (const option of options) {
      ...
      assert.equal(fits(game.board, { ...game.piece, ...option, y: option.y + 1 }), false)
    }
...
test('speed increases with active time and request validation rejects invalid data', () => {
  ...
  const valid = { game: 'tetris', decisionId: '1:0', board: createGame([0]).board, kind: 0, next: [1, 2, 3] }
  assert.equal(decisionRequest.safeParse(valid).success, true)
  assert.equal(decisionRequest.safeParse({ ...valid, board: [] }).success, false)
  assert.equal(decisionRequest.safeParse({ ...valid, apiKey: 'unexpected' }).success, false)
})
